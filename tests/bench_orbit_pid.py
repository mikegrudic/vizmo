    ...
    def step(self, last_render_ms, cap, ceiling, dt_s=None):
        if last_render_ms > 0:
            self.smooth_ms = ((1 - self.alpha) * self.smooth_ms + self.alpha * last_render_ms
                              if self.smooth_ms > 0 else last_render_ms)
        if self.smooth_ms <= 0:
            return cap
...
class TimeBased:
    ...
    """
    def __init__(self, target_ms, tau_smooth_s=0.15, tau_resp_s=0.4,
                 max_rate_log2=2.0):
        self.target_ms = target_ms
        ...
        self.smooth_ms = 0.0
        self.name = (f"time(tau_s={tau_smooth_s:.2f},tau_r={tau_resp_s:.2f},"
                     f"rate={max_rate_log2:.1f}/s)")
...
def run_orbit(label, device, r, cam, target_center, ctrl, *,
              res, duration_s, target_fps, ceiling,
              orbit_radius, orbit_period_s, edge_radius, n_passes):
    ...
    while True:
        ...
        offset = np.array([radius * np.cos(ang),
                           0.0,
                           radius * np.sin(ang)], dtype=np.float32)
...
def summarize(label, recs):
    ...
    print(f"  ms:   mean {ms.mean():6.2f}  median {np.median(ms):6.2f}  "
          f"std {ms.std():6.2f}  p5 {np.percentile(ms,5):6.2f}  p95 {np.percentile(ms,95):6.2f}")
    print(f"  cap:  mean {cap.mean()/1e6:6.2f}M  std {cap.std()/1e6:6.2f}M  "
...
def main():
    ...
    p.add_argument("--target-fps", type=float, nargs="+",
                   default=[15.0, 30.0, 60.0],
                   help="one or more target FPS values to test")
    p.add_argument("--duration", type=float, default=10.0)
    p.add_argument("--orbit-period", type=float, default=8.0)
    p.add_argument("--orbit-radius-frac", type=float, default=0.05,
                   help="inner orbit radius as a fraction of the data extent")
    p.add_argument("--edge-radius-frac", type=float, default=0.45,
                   help="outer (edge) radius as a fraction of the data extent")
    p.add_argument("--radial-passes", type=int, default=3,
                   help="number of full in-out cycles over the run")
    p.add_argument("--ceiling", type=int, default=0,
                   help="cap ceiling (0 = full N)")
    args = p.parse_args()
    ...
    for fps in args.target_fps:
        ...
        controllers = [
            ...
            ("time(0.10/0.25)", TimeBased(target_ms,
                                          tau_smooth_s=0.10, tau_resp_s=0.25,
                                          max_rate_log2=3.0)),
            ("time(0.07/0.15)", TimeBased(target_ms,
                                          tau_smooth_s=0.07, tau_resp_s=0.15,
                                          max_rate_log2=4.0)),
            ("time(0.05/0.10)", TimeBased(target_ms,
                                          tau_smooth_s=0.05, tau_resp_s=0.10,
                                          max_rate_log2=5.0)),
        ]
        print(f"\n--- target {fps:.0f} fps (target_ms={target_ms:.2f}) ---")
        for label, ctrl in controllers:
            r.set_subsample_max_per_frame(ceiling)
            recs = run_orbit(label, device, r, cam, center, ctrl,
                             res=args.res, duration_s=args.duration,
                             target_fps=fps, ceiling=ceiling,
                             orbit_radius=orbit_r, orbit_period_s=args.orbit_period,
                             edge_radius=edge_r, n_passes=args.radial_passes)
    ...
    print(f"{'fps':>5} {'controller':<48} {'mean ms':>10} {'ms std':>10} "
          f"{'p95 ms':>10} {'cap step':>10}")
    for fps, name, s in all_results:
        log_step = np.log2(s["cap"][1:] / np.maximum(s["cap"][:-1], 1)).std()
        print(f"{fps:>5.0f} {name:<48} {s['ms'].mean():>10.2f} "
